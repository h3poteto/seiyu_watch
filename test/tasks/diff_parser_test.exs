defmodule SeiyuWatch.DiffParserTest do
  ...
  describe "#wiki_page_request" do
    ...
    test "get request has revision hash when seiyu has differences", %{seiyu: seiyu} do
      with_mock SeiyuWatch.WikipediaResponse, [:passthrough], [get_response: fn(_request) -> response_page(%{revid: 345678}) end] do
        {:ok, prev_diff} = seiyu
        |> Ecto.build_assoc(:differences)
        |> SeiyuWatch.Difference.changeset(%{wiki_diff: "some diff", from: 123456, to: 234567})
        |> Repo.insert
        differences = SeiyuWatch.Seiyu
        |> Repo.get!(seiyu.id)
        |> Repo.preload(:differences)
  ...
  describe "#update_diff" do
    setup do
      ...
      %{seiyu: seiyu, current_revision_request: current_revision_request, page_request: page_request}
    end
    test "when seiyu has no differences", %{seiyu: seiyu, current_revision_request: current_revision_request, page_request: page_request} do
      with_mocks([
        {SeiyuWatch.WikipediaResponse,
         [:passthrough],
         [get_response: fn(^current_revision_request) -> response_page(%{revid: 345678}) end,
          get_response: fn(^page_request) -> response_diff(%{diff: "some diff", from: 234567, to: 345678}) end]},
        {SeiyuWatch.DifferenceEvent,
         [],
