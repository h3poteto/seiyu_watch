defmodule SeiyuWatch.WikiParserTest do
  use SeiyuWatch.TaskCase

  import Mock
  import SeiyuWatch.WikipediaResponseHelpers
  alias SeiyuWatch.Workers.WikiParser
  ...
  describe "#update_wiki" do
    ...
    test "when seiyu does not have wikipedia record", %{seiyu: seiyu} do
      with_mock SeiyuWatch.WikipediaResponse, [:passthrough],
        get_response: fn _request -> response_content(%{content: "content"}) end do
        refute Repo.get_by(SeiyuWatch.Wikipedia, seiyu_id: seiyu.id)
        _wikipedia = WikiParser.update_wiki(seiyu.id)
    ...
    test "when seiyu has wikipedia record", %{seiyu: seiyu} do
      with_mock SeiyuWatch.WikipediaResponse, [:passthrough],
        get_response: fn _request -> response_content(%{content: "new content"}) end do
        seiyu
